...
try {
    ...
    // 方法1: 從 Session 取得 (如果你的 login.php 有設定)
    if (isset($_SESSION['employee_name']) && !empty($_SESSION['employee_name'])) {
        $employeeName = $_SESSION['employee_name'];
        $employeeId = $_SESSION['employee_id'] ?? null;
    }
    // 方法2: 如果沒有 Session,從資料庫取得第一個員工 (測試用)
    else {
        $testQuery = $conn->query("SELECT id, name FROM 員工基本資料 LIMIT 1");
        if ($testQuery && $testQuery->num_rows > 0) {
            $testUser = $testQuery->fetch_assoc();
