...
$searchField = $_GET["searchField"] ?? "name"; // 預設欄位
$result = null;
...
$sql = "SELECT id, name, role, Position, base_salary, hourly_rate,
               Telephone, address, ID_card, account
        FROM `員工基本資料`";

if (!empty($keyword) && array_key_exists($searchField, $allowedFields)) {
    $likeKeyword = "%" . $keyword . "%";
...
<form method="get" action="employee_search_select.php">
...
<?php if ($result !== null): ?>
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<tr><td colspan='10'>查無資料</td></tr>";
        }
        ?>
    </table>
<?php endif; ?>
